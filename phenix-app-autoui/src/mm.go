...
func getFileFromHost(hostName, srcPath, dstPath string) error {
	...
	if err := scpCmd.Run(); err != nil {
		fmt.Println(err)
		return fmt.Errorf("Can not get %s from %s:%v", srcPath, hostName, err)
	}
...
func takeScreenshot(expName, vmName, dstPath string) error {
	...
	if err := mmcli.ErrorResponse(mmcli.Run(cmd)); err != nil {
		fmt.Printf("Can not take screenshot of VM %s in namespace %s: %v", vmName, expName, err)
		return fmt.Errorf("Can not take screenshot of VM %s in namespace %s: %v", vmName, expName, err)
	}
