...
// template.
func GenerateFromTemplate(path string, data interface{}, w io.Writer) error {
	...
	tmpl := template.Must(template.New(filepath.Base(path)).Funcs(funcs).ParseFiles(path))

	if err := tmpl.Execute(w, data); err != nil {
		return fmt.Errorf("executing %s template: %w", path, err)
	}
...
func CreateFileFromTemplate(path string, data interface{}) error {
	...
	return GenerateFromTemplate(path, data, f)
}
